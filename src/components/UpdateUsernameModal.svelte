  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (finalUsername && finalUsername !== formattedUsername) {
        success(
          `Username updated to ${result.lightningAddress} (adjusted for uniqueness)`,
        );
      } else {
  ...
  <div class="modal-content" onclick={(e) => e.stopPropagation()} role="dialog">
    {#if !showConfirmation}
      ...
      <div class="modal-body">
        <p class="text-sm text-white/60 mb-4">
          Current: <span class="text-white font-medium"
            >{currentUsername}@{domain}</span
          >
        </p>
    ...
    {:else}
      ...
      <div class="modal-body">
        <!-- Big Warning -->
        <div
          class="bg-red-500/20 border-2 border-red-500/50 rounded-lg p-4 mb-4"
        >
          <div class="flex items-start gap-3">
            ...
            <div class="flex-1">
              <p class="font-bold text-red-400 mb-2">
                ⚠️ Your old Lightning address will STOP working!
              </p>
              <p class="text-sm text-white/80 mb-3">
                After this update, payments sent to your old address will NOT
                reach you.
              </p>
            </div>
          </div>
        ...
        <div class="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
          <p class="text-sm text-amber-400 font-medium mb-1">
            📢 Tell your friends!
          </p>
          <p class="text-xs text-white/70">
            Make sure to notify anyone who sends you payments about your new
            Lightning address.
          </p>
        </div>
      </div>
